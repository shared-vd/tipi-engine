...
public class StopStartGroupActivity3 extends Activity {

    public static AtomicInteger beginStep = new AtomicInteger(0);
    public static AtomicInteger endStep = new AtomicInteger(0);

    public static final ActivityMetaModel meta = new ActivityMetaModel(StopStartGroupActivity3.class);

    @Override
    public ActivityResultContext execute() throws Exception {
        beginStep.incrementAndGet();

        while (StopStartGroupParentProcess.globalStep.get() < 4) {
            Thread.sleep(10);
        }

...
public class ActivityModel extends TpBaseEntity {
    ...
    private TopProcessModel process;
    private SubProcessModel parent;
    private ActivityModel previous;
    ...
    public TopProcessModel getProcess() {
        return process;
    }

    public void setProcess(TopProcessModel process) {
    ...
    @Transient
    public TopProcessModel getProcessOrThis() {
        if (process == null) {
            // Si on n'a pas de process, on EST le process.
            return (TopProcessModel) this;
        }
        return process;
    }
    ...
    @OnDelete(action = OnDeleteAction.CASCADE)
    public SubProcessModel getParent() {
        return parent;
    }

    public void setParent(SubProcessModel parent) {
        this.parent = parent;
    }
    ...
    public String getProcessName() {
        if (this instanceof TopProcessModel) {
            // Si on n'a pas de process, on EST le process.
            return fqn;
        }
        return processName;
    ...
    @OnDelete(action = OnDeleteAction.CASCADE)
    public ActivityModel getPrevious() {
        return previous;
    }

    public void setPrevious(ActivityModel previous) {
    ...
    private void setVariables(Map<String, Variable<?>> variables) {
        this.variables = variables;
    }

    // === Transient ===

    public void putVariable(Variable<?> aVar) {
    ...
    public Object getVariable(String key) {
        Variable<?> var = getVariables().get(key);
        if (var != null) {
            return var.getValue();
        }
        return null;
